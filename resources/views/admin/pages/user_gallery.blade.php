                    ...
                    <div style="margin-top:10px">
                        <div class="page-header">
                            ...
                            <div class=" d-flex float-end">
                                <button class="btn btn-l btn-success text-light me-2" id="slide_show">
                                    <span class="mdi fs-5 text-black mdi-play-box-outline"></span>
                                </button>
                                <button class=" btn btn-l btn-success text-light" id="download">
                                    <span class="mdi fs-5 text-black mdi-download">
                                    </span>
                                </button>
                            </div>
                        </div>
                        ...
                        <div class="card-body">
                            <div class="card-title d-flex justify-content-center">
                                <form method="post" id="image_form" onsubmit="return false" enctype="multipart/form-data">
                                    <input style="width:50%" class="d-inline-block form-control remove-text " type="file"
                ...
                success: function(response) {
                    ...
                    if (response.images.length) {
                        for (let index = 0; index < response.images.length; index++) {
                            ...
                            <img id="user_image" class="d-block" src="{{ url('user_image') }}/` + response.images[
                                    index].image + `">
                            ...
                            <span class="mdi text-black mdi-window-close"></span></button>
                            <span id="` + response.images[index].id +
                                `_title" class="text-nowrap title  fw-bold text-center overflow-hidden text-truncate align-items-center d-inline-block"></span>
                            <span class="d-inline-block text-nowrap  description w-50 align-self-center text-center overflow-hidden text-truncate " id="` +
                                response.images[index].id + `_desc"></span></div>`)

                            $('#' + response.images[index].id + '_title').html(response.images[index]
                                .title)
                            $('#' + response.images[index].id + '_desc').html(response.images[index]
                                .description)
                ...
                success: function(response) {
                    ...
                    for (let index = 0; index < response.images.length; index++) {
                        $('#display_images').append(`<div class="main-div flex-column d-inline-flex position-relative align-top">
                            ...
                            <img id="user_image" class="d-block" src="{{ url('user_image') }}/` + response.images[
                                index] + `">
                            </div><button onclick="delete_image(` + response.id[index] + `,this)"  class="delete_btn top-0 end-0 bg-danger position-absolute rounded-circle"></a><span class="mdi text-black mdi-window-close"></span><i class="text-light fa fa-times" aria-hidden="true"></i></button>
                            <span id="` + response.id[index] +
                            `_title" class="text-nowrap title text-light fw-bold text-center overflow-hidden text-truncate align-items-center d-inline-block"></span>
                            <span class="d-inline-block text-nowrap text-light description w-50 align-self-center text-center overflow-hidden text-truncate " id="` +
                            response.id[index] + `_desc"></span></div>`)
                    ...
                    if (response.images.length) {
                        for (let i = 0; i < response.images.length; i++) {
                            ...
                            $('.slideshow-container').append(
                                '<div class="mySlides w-100 justify-content-center flex-column" ><img class="slide_image" src={{ url('user_image') }}/' +
                                response.images[i].image +
                                ' style="margin:auto"><div class="text text-light fs-5 text-center text-truncate"><span>' +
                                text + '</span><br><span>' + text2 + '</span></div>')
